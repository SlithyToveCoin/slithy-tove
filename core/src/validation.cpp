...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Slithy Tove subsidy is cut in half according to the active network interval.
    nSubsidy >>= halvings;
    return nSubsidy;
}
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    CAmount blockSubsidy = GetBlockSubsidy(pindex->nHeight, consensus_params);
    CAmount blockReward = nFees + blockSubsidy;
    if (block.vtx[0]->GetValueOut() > blockReward && state.IsValid()) {
        state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount",
                      strprintf("coinbase pays too much (actual=%d vs limit=%d)", block.vtx[0]->GetValueOut(), blockReward));
    }
    if (pindex->nHeight > 0 && !consensus_params.treasury_script_pub_key.empty() && state.IsValid()) {
        const CScript treasury_script{consensus_params.treasury_script_pub_key.begin(), consensus_params.treasury_script_pub_key.end()};
        const CAmount expected_treasury_reward{blockSubsidy * consensus_params.treasury_percent / 100};
        CAmount actual_treasury_reward{0};
        for (const auto& output : block.vtx[0]->vout) {
            if (output.scriptPubKey == treasury_script) {
                actual_treasury_reward += output.nValue;
            }
        }
        if (actual_treasury_reward != expected_treasury_reward) {
            state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-treasury",
                          strprintf("coinbase treasury payment incorrect (actual=%d vs required=%d)", actual_treasury_reward, expected_treasury_reward));
        }
    }
    if (control) {
