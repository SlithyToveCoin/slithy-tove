...
{
    InitializeForNetwork(their_pubkey, initiator, Params().MessageStart(), self_decrypt);
}

void BIP324Cipher::InitializeForNetwork(const EllSwiftPubKey& their_pubkey, bool initiator,
                                      const MessageStartChars& message_header, bool self_decrypt) noexcept
{
