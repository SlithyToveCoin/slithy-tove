// Copyright (c) 2017-present The Slithy Tove Core developers
...
CFeeRate ParseFeeRate(const UniValue& json)
{
    ...
    if (val >= COIN) throw JSONRPCError(RPC_INVALID_PARAMETER, "Fee rates larger than or equal to 1SLTHY/kvB are not accepted");
    return CFeeRate{val};
}
...
{
    return "> slithy-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleCliNamed(const std::string& methodname, const RPCArgList& args)
{
    std::string result = "> slithy-cli -named " + methodname;
