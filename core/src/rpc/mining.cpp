...
// Copyright (c) 2026 The Slithy Tove developers
...
namespace {
std::recursive_mutex g_slithy_cpu_transition_mutex;
std::mutex g_slithy_cpu_miner_mutex;
std::atomic<uint64_t> g_slithy_work_id{0};
std::atomic<int> g_slithy_live_workers{0};
std::vector<std::thread> g_slithy_cpu_miner_threads;
std::atomic_bool g_slithy_cpu_miner_stop{false};
std::atomic<uint64_t> g_slithy_cpu_miner_hashes{0};
std::atomic<uint64_t> g_slithy_cpu_miner_blocks{0};
std::chrono::steady_clock::time_point g_slithy_cpu_miner_started;
std::string g_slithy_cpu_miner_address;

void SlithyCpuMinerThread(NodeContext* node, CScript coinbase_output_script)
{
    // Joining happens in Shutdown before the node context is destroyed.
    struct WorkerLifetime {
        ~WorkerLifetime() { g_slithy_live_workers.fetch_sub(1); }
    } lifetime;
    try {
    Mining& miner = *Assert(node->mining);
    ChainstateManager& chainman = *Assert(node->chainman);

    while (!g_slithy_cpu_miner_stop.load(std::memory_order_relaxed) && !chainman.m_interrupt) {
        ...
        // Each template gets its own coinbase, so workers never repeat the
        // same nonce search even when their timestamps and transactions match.
        CMutableTransaction coinbase{*block.vtx[0]};
        const uint64_t work_id = g_slithy_work_id.fetch_add(1);
        // Separate computers may mine to the same wallet at the same time.
        // Random work data prevents those processes from duplicating a search.
        const uint256 work_nonce = GetRandHash();
        coinbase.vin[0].scriptSig << std::vector<unsigned char>(work_nonce.begin(), work_nonce.end());
        coinbase.vin[0].scriptSig << static_cast<int64_t>(work_id);
        block.vtx[0] = MakeTransactionRef(std::move(coinbase));
        block.hashMerkleRoot = BlockMerkleRoot(block);
        LogDebug(BCLog::VALIDATION, "Slithy CPU work id=%u parent=%s merkle=%s\n", work_id, block.hashPrevBlock.ToString(), block.hashMerkleRoot.ToString());
        auto last_tip_check = std::chrono::steady_clock::now();
        const auto template_started = last_tip_check;

        while (!g_slithy_cpu_miner_stop.load(std::memory_order_relaxed) &&
               !chainman.m_interrupt &&
               block.nNonce < std::numeric_limits<uint32_t>::max()) {
            const auto now = std::chrono::steady_clock::now();
            if (now - last_tip_check >= std::chrono::milliseconds{250}) {
                LOCK(cs_main);
                const CBlockIndex* tip = chainman.ActiveChain().Tip();
                if (!tip || SlithyCpuWorkExpired(block.hashPrevBlock, tip->GetBlockHash(), std::chrono::duration_cast<std::chrono::milliseconds>(now - template_started))) {
                    LogDebug(BCLog::VALIDATION, "Slithy CPU work id=%u discarded after tip change or template expiry\n", work_id);
                    break;
                }
                last_tip_check = now;
            }
            // A fresh template also refreshes time and transaction selection.
            if (now - template_started >= std::chrono::seconds{5}) break;
            g_slithy_cpu_miner_hashes.fetch_add(1, std::memory_order_relaxed);
            if (CheckProofOfWork(block.GetHash(), block.nBits, chainman.GetConsensus())) {
                ...
                if (chainman.ProcessNewBlock(block_out, /*force_processing=*/true, /*min_pow_checked=*/true, &new_block) && new_block) {
                    LOCK(cs_main);
                    const CBlockIndex* accepted = chainman.m_blockman.LookupBlockIndex(block_out->GetHash());
                    if (accepted && chainman.ActiveChain().Contains(accepted))
                        g_slithy_cpu_miner_blocks.fetch_add(1, std::memory_order_relaxed);
                }
                break;
            }
        ...
        LogError("CPU miner stopped: %s", error.what());
    } catch (...) {
        LogError("CPU miner stopped after an unexpected worker error.");
    }
}

bool SlithyCpuMinerActive()
{
    std::lock_guard<std::mutex> lock(g_slithy_cpu_miner_mutex);
    return g_slithy_live_workers.load() > 0;
}

int SlithyCpuMinerThreadCount()
{
    std::lock_guard<std::mutex> lock(g_slithy_cpu_miner_mutex);
    return g_slithy_live_workers.load();
}

std::string SlithyCpuMinerAddress()
{
    std::lock_guard<std::mutex> lock(g_slithy_cpu_miner_mutex);
    return g_slithy_cpu_miner_address;
}

uint64_t SlithyCpuMinerHashrate()
{
    std::chrono::steady_clock::time_point started;
    {
        std::lock_guard<std::mutex> lock(g_slithy_cpu_miner_mutex);
        started = g_slithy_cpu_miner_started;
    }

    const auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
        std::chrono::steady_clock::now() - started).count();
    if (elapsed <= 0) return 0;
    return g_slithy_cpu_miner_hashes.load(std::memory_order_relaxed) / static_cast<uint64_t>(elapsed);
}

void StopSlithyCpuMinerInternal()
{
    std::lock_guard<std::recursive_mutex> transition(g_slithy_cpu_transition_mutex);
    std::vector<std::thread> threads;
    {
        std::lock_guard<std::mutex> lock(g_slithy_cpu_miner_mutex);
        if (g_slithy_cpu_miner_threads.empty()) return;
        g_slithy_cpu_miner_stop.store(true, std::memory_order_relaxed);
        threads.swap(g_slithy_cpu_miner_threads);
    }
...
static RPCHelpMan startcpumining()
{
    return RPCHelpMan{"startcpumining",
        "Start Slithy CPU mining in this node process. This is local nonce hashing, not developer block generation.",
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to receive miner rewards."},
            {"threads", RPCArg::Type::NUM, RPCArg::Default{1}, "Number of CPU mining threads."},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::BOOL, "active", "Whether CPU mining is active."},
                {RPCResult::Type::NUM, "threads", "Number of active mining threads."},
                {RPCResult::Type::STR, "address", "Reward address."},
            }
        },
        RPCExamples{
            HelpExampleCli("startcpumining", "\"tslithy1...\" 2")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    int threads = std::max(1, request.params[1].isNull() ? 1 : request.params[1].getInt<int>());
    threads = std::min(threads, 64);

    NodeContext& node = EnsureAnyNodeContext(request.context);
    CScript coinbase_output_script = GetScriptForDestination(destination);

    const auto& consensus = EnsureChainman(node).GetConsensus();
    if (EnsureChainman(node).m_interrupt)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "The node is shutting down.");
    const CScript treasury{consensus.treasury_script_pub_key.begin(), consensus.treasury_script_pub_key.end()};
    if (!treasury.empty() && coinbase_output_script == treasury)
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Choose a mining address other than the literacy treasury.");
    std::lock_guard<std::recursive_mutex> transition(g_slithy_cpu_transition_mutex);
    StopSlithyCpuMiner();

    {
        std::lock_guard<std::mutex> lock(g_slithy_cpu_miner_mutex);
        g_slithy_cpu_miner_stop.store(false, std::memory_order_relaxed);
        g_slithy_cpu_miner_hashes.store(0, std::memory_order_relaxed);
        g_slithy_cpu_miner_blocks.store(0, std::memory_order_relaxed);
        g_slithy_cpu_miner_started = std::chrono::steady_clock::now();
        g_slithy_cpu_miner_address = address;
        g_slithy_cpu_miner_threads.reserve(threads);
        for (int i = 0; i < threads; ++i) {
            g_slithy_live_workers.fetch_add(1);
            try {
                g_slithy_cpu_miner_threads.emplace_back(SlithyCpuMinerThread, &node, coinbase_output_script);
            } catch (...) {
                g_slithy_live_workers.fetch_sub(1);
                g_slithy_cpu_miner_stop.store(true);
                for (auto& worker : g_slithy_cpu_miner_threads) if (worker.joinable()) worker.join();
                g_slithy_cpu_miner_threads.clear();
                throw;
            }
        }
    }
...
static RPCHelpMan stopcpumining()
{
    return RPCHelpMan{"stopcpumining",
        "Stop Slithy CPU mining in this node process.",
        {},
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::BOOL, "active", "Whether CPU mining is still active."},
            }
        },
        RPCExamples{
            HelpExampleCli("stopcpumining", "")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    StopSlithyCpuMiner();
...
static RPCHelpMan getcpumininginfo()
{
    return RPCHelpMan{"getcpumininginfo",
        "Return Slithy CPU miner status.",
        {},
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::BOOL, "active", "Whether CPU mining is active."},
                {RPCResult::Type::NUM, "speed", "Approximate hashes per second since mining started."},
                {RPCResult::Type::NUM, "threads", "Number of active mining threads."},
                {RPCResult::Type::STR, "address", "Reward address."},
                {RPCResult::Type::NUM, "blocks", "Blocks found by this miner since start."},
                {RPCResult::Type::STR, "algorithm", "Mining algorithm."},
            }
        },
        RPCExamples{
            HelpExampleCli("getcpumininginfo", "")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    UniValue result(UniValue::VOBJ);
    result.pushKV("active", SlithyCpuMinerActive());
    result.pushKV("speed", SlithyCpuMinerHashrate());
    result.pushKV("threads", SlithyCpuMinerThreadCount());
    result.pushKV("address", SlithyCpuMinerAddress());
    result.pushKV("blocks", g_slithy_cpu_miner_blocks.load(std::memory_order_relaxed));
    result.pushKV("algorithm", "yespower");
    return result;
},
    };
}
...
// NOTE: Unlike wallet RPC (which use SLTHY values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
    ...
    return RPCHelpMan{
        ...
        },
        {
            ...
            RPCResult{"Otherwise", RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::OBJ, "slithy_treasury", "mandatory treasury output that must be included unchanged in the coinbase transaction",
                {
                    {RPCResult::Type::NUM, "value", "required treasury amount in satoshis"},
                    {RPCResult::Type::STR_HEX, "scriptPubKey", "required treasury output script"},
                    {RPCResult::Type::NUM, "percent", "consensus treasury percentage of the block reward"},
                }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!lpval.isNull()) {
         ...
         *
         * getblocktemplate is unlikely to be called by slithy-cli, so
    ...
    result.pushKV("coinbasevalue", coinbase_value);

    const CScript treasury_script{
        consensusParams.treasury_script_pub_key.begin(),
        consensusParams.treasury_script_pub_key.end()};
    const auto treasury_output = std::find_if(
        block.vtx[0]->vout.begin(),
        block.vtx[0]->vout.end(),
        [&](const CTxOut& output) { return output.scriptPubKey == treasury_script; });
    CHECK_NONFATAL(treasury_output != block.vtx[0]->vout.end());
    UniValue treasury(UniValue::VOBJ);
    treasury.pushKV("value", treasury_output->nValue);
    treasury.pushKV("scriptPubKey", HexStr(treasury_output->scriptPubKey));
    treasury.pushKV("percent", consensusParams.treasury_percent);
    result.pushKV("slithy_treasury", std::move(treasury));
...
void RegisterMiningRPCCommands(CRPCTable& t)
{
    static const CRPCCommand commands[]{
        ...
        {"mining", &startcpumining},
        {"mining", &stopcpumining},
