...
const char * const BITCOIN_CONF_FILENAME = "slithy.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Slithy
    //   new: C:\Users\Username\AppData\Local\Slithy
    // macOS: ~/Library/Application Support/Slithy
    // Unix-like: ~/.slithy
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Slithy";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Slithy";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Slithy";
#else
    // Unix-like
    return pathRet / ".slithy";
#endif
#endif
}
