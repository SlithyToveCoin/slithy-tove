...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock()
{
    ...
    // Add coinbase outputs. Fees remain with the miner. Slithy Tove sends
    // the configured treasury percent of the subsidy to the literacy treasury.
    const auto& consensus_params{chainparams.GetConsensus()};
    const CAmount block_subsidy{GetBlockSubsidy(nHeight, consensus_params)};
    const CAmount treasury_reward{consensus_params.treasury_script_pub_key.empty() ? 0 : block_subsidy * consensus_params.treasury_percent / 100};
    const CAmount miner_reward{nFees + block_subsidy - treasury_reward};
    const CAmount block_reward{nFees + block_subsidy};

    coinbaseTx.vout.resize(treasury_reward > 0 ? 2 : 1);
    coinbaseTx.vout[0].scriptPubKey = m_options.coinbase_output_script;
    coinbaseTx.vout[0].nValue = miner_reward;
    if (treasury_reward > 0) {
        coinbaseTx.vout[1].scriptPubKey = CScript{consensus_params.treasury_script_pub_key.begin(), consensus_params.treasury_script_pub_key.end()};
        coinbaseTx.vout[1].nValue = treasury_reward;
    }
                                                      ...
                                                      bool& interrupt_wait)
{
    ...
    do {
        ...
        // Refresh after the active network's minimum-difficulty delay.
        if (!tip_changed && allow_min_difficulty) {
            const NodeClock::time_point tip_time{std::chrono::seconds{chainman.ActiveChain().Tip()->GetBlockTime()}};
            if (now > tip_time + std::chrono::seconds{2 * chainman.GetConsensus().nPowTargetSpacing}) {
                tip_changed = true;
            }
        }
